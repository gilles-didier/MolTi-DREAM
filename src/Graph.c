...
double getModularityMultiWeighted(TypeMultiGraph *g, TypePartition *part, double *weight, double alpha) {
	double res = 0., *k, m;
	TypePartitionList *pl;
	int t, c, i, j;
	...
	for(t=0; t<g->sizeTable; t++) {
		...
		for(c=0; c<part->sizeAtom; c++) {
			...
			for(e=pl->start[c]; e>=0; e=pl->next[e])
				for(f=pl->next[e]; f>=0; f=pl->next[f])
					res += weight[t]*((double) g->edge[t][e][f]-(alpha*k[e]*k[f])/(2.*m))/(2.*m);
		}
	}
	free((void*)k);
	freePartitionList(pl);
...
TypeGraph *readGraph(FILE *f) {
	...
	for(c = fgetc(f); c != EOF && issepline(c); c = fgetc(f));
	while(c != EOF) {
		char tmpS[MAX_NAME_SIZE+1], tmpE[MAX_NAME_SIZE+1], tmpV[;
		int i, indexS, indexE;
		if(c == '\'' || c == '"') {
			c = fgetc(f);
			for(i=0; i<MAX_NAME_SIZE && c != EOF && c != '\'' && c != '"'; i++) {
...
TypeGraph *readGraph(FILE *f) {
	...
	val = (double*)malloc(sizebuf*sizeof(double));
	sizeGraph = 0;
	for(c = fgetc(f); c != EOF && issepline(c); c = fgetc(f));
	while(c != EOF) {
		char tmpS[MAX_NAME_SIZE+1], tmpE[MAX_NAME_SIZE+1], tmpV[MAX_NAME_SIZE+1];
		int i, indexS, indexE;
		if(c == '\'' || c == '"') {
			c = fgetc(f);
			for(i=0; i<MAX_NAME_SIZE && c != EOF && c != '\'' && c != '"'; i++) {
...
//printf("%s-%d\n", tmp, indexE);
				if(sizeGraph >= sizebuf) {
					...
					val =(double*) realloc((void*)val, sizebuf*sizeof(double));
				}
				for(; c != EOF && issep(c); c = fgetc(f));
                for(i=0; i<MAX_NAME_SIZE && c != EOF && !issepline(c); i++) {
					tmpV[i] = c;
	...
	for(n=0; n<sizeGraph; n++) {
        g->edge[start[n]][end[n]] = UTILS_MAX(g->edge[start[n]][end[n]], val[n]);
		g->edge[end[n]][start[n]] = g->edge[start[n]][end[n]];
	}
	free((void*)start);
	free((void*)end);
	free((void*)val);
...
/*print graph in standard format*/	
void fprintGraphSpecial(FILE *f, TypeGraph *g) {
	TypeSizeG n, m;
	for(n=0; n<g->sizeGraph; n++)
		for(m=0; m<n; m++) {
			if(g->edge[n][m]) {
				if(g->name != NULL)
					fprintf(f, "%s\t%s\t%lf\n", g->name[n], g->name[m], g->edge[n][m]);
				else
					fprintf(f, "%d\t%d\t%lf\n", n, m, g->edge[n][m]);
			}
		}
...
/*print graph in standard format*/	
void fprintWeightedGraph(FILE *f, TypeGraph *g) {
	TypeSizeG n, m;
	for(n=0; n<g->sizeGraph; n++)
		for(m=0; m<n; m++) {
			if(g->edge[n][m]>0) {
